...
class ProductsDBC {
    ...
    public ProductList productsSugesstionTo(Product singleProduct) {
        String sqlStart = "SELECT * FROM `products`";
        String sql = "";
        String sqlInnerJoinGenre = "";
        int currentSqlSize = 0;
        String productId = singleProduct.getIdProduct() + "";
        singleProduct.getGenres();
        singleProduct.getPublisher();

        if (singleProduct.getGenres().size() != 0) {
            sqlInnerJoinGenre = "INNER JOIN genre_product ON products.id_products = genre_product.id_product ";
            for (String genre : singleProduct.getGenres()) {
                sql += " genre_product.id_genre = (SELECT id_genre FROM genre WHERE name = '" + genre + "') OR ";
            }
            sql = sql.substring(0, sql.length() - 3);
            currentSqlSize = sql.length();
        }
        if (singleProduct.getPublisher() != null) {
            if (currentSqlSize != 0) {
                sql += " OR ";
            }
            sql += " products.id_publisher = (SELECT id_publisher FROM publisher WHERE name = '" + singleProduct.getPublisher() + "') OR ";
            sql = sql.substring(0, sql.length() - 3);
        }
        if (singleProduct.getProducer() != null) {
            if (currentSqlSize != 0) {
                sql += " OR ";
            }
            sql += " products.id_producer = (SELECT id_producer FROM producer WHERE name = '" + singleProduct.getProducer() + "') OR ";
            sql = sql.substring(0, sql.length() - 3);
        }

        if(sql.length() != 0){
            sql = sqlStart + sqlInnerJoinGenre + " WHERE products.id_products  NOT LIKE "+ productId + " AND (" + sql + ")";
        }else{
            sql = sqlStart +" WHERE name = 'c++' ";
        }
